...
export function Footer() {
  ...
  return (
    <footer className="bg-[var(--bg-secondary)] border-t border-[var(--border-color)] px-8 lg:px-16 pt-14 pb-8">
      <div className="max-w-7xl mx-auto">
        {/* Top */}
        <div className="flex flex-col md:flex-row justify-between gap-10 mb-14">
          <div className="max-w-xs">
            <a href="#hero" className="font-syne font-black text-2xl gradient-text block mb-3">⟨IS/⟩</a>
          ...
          <div className="flex gap-16">
            {NAV_COLS.map((col) => (
              <div key={col.title}>
                <h4 className="text-xs font-semibold text-[var(--text-primary)] tracking-[0.08em] uppercase mb-4">{col.title}</h4>
            ...
            <div>
              <h4 className="text-xs font-semibold text-[var(--text-primary)] tracking-[0.08em] uppercase mb-4">Connect</h4>
              <ul className="space-y-3">
                {portfolioData.socials.map((s) => (
                  <li key={s.platform}>
                    <a
        ...
        <div className="border-t border-[var(--border-color)] pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-[var(--text-tertiary)]">
            ...
            <Heart size={10} className="inline text-[var(--accent-primary)]" /> in Indonesia.
          </p>
          <p className="text-xs text-[var(--text-tertiary)]">
